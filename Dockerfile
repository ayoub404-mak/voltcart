...
# ✅ FIX 1: Install the Prisma CLI so prisma.config.ts can resolve 'prisma/config'
...
# ✅ FIX 2: Copy the Prisma config file so the CLI can find the database URL at runtime!
# (Note: Use prisma.config.ts or prisma.config.js depending on your actual file extension)
...
# Use the entrypoint script to run migrations then start the app
