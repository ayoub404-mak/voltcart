...
import toast from "react-hot-toast" // ✅ FIX: Changed to default import for consistency
import { useDispatch } from "react-redux"
import { useAuth } from "@clerk/nextjs" // ✅ ADDED: Missing import
...
const AddressModal = ({ setShowAddressModal }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.log(error)
            // ✅ FIX: Changed .message to .error to match the backend route we fixed earlier
    ...
    return (
        ...
        <form onSubmit={handleSubmit} className="fixed inset-0 z-50 bg-white/60 backdrop-blur h-screen flex items-center justify-center">
            <div className="flex flex-col gap-5 text-slate-700 w-full max-w-sm mx-6">
                ...
                <button type="submit" className="bg-slate-800 text-white text-sm font-medium py-2.5 rounded-md hover:bg-slate-900 active:scale-95 transition-all">
                    SAVE ADDRESS
                </button>
            </div>
